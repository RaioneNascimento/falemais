class Calculation {
  async store(req, res) {
    ...
    const pricePerMinute = (origin, to) => {
      ...
      if (origin === '016' && to === '011') return 1.9;
      if (origin === '011' && to === '017') return 1.9;
      if (origin === '017' && to === '011') return 1.9;
      if (origin === '011' && to === '018') return 1.9;
    ...
    let withFaleMais = () => {
      ...
      else { withFaleMais = (time - fare) * (pricePerMinute(from, destination) + 0.1 * pricePerMinute(from, destination))        
